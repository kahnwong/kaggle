...
for i in x_cols_categorial:
    ...
    # values = pd.Series(['red', 'green', 'blue', 'red', 'blue']).values.reshape(-1, 1)
    # print(values)
    enc.fit(values)
    encs[i] = enc
    ...
    one_hot_df = pd.DataFrame(one_hot_encoded_data, columns=enc.get_feature_names_out(['category']))
...
for i in x_cols_categorial:
    ...
    one_hot_df = pd.DataFrame(one_hot_encoded_data, columns=encs[i].get_feature_names_out(['category']))
    encoded_series = one_hot_df.idxmax(axis=1)

    # map these column names to a simple numeric value
    mapping = {name: i for i, name in enumerate(one_hot_df.columns)}
    numeric_series = encoded_series.map(mapping)
    df_test_encoded = df_test_encoded.with_columns(pl.Series(numeric_series.to_list()).alias(i))
...
# evaluate
df_ground_truth = pl.read_csv('data/gender_submission.csv').rename({'Survived': 'Survived_ground_truth'})

df_prediction = df_prediction.join(df_ground_truth, on=id_col, how='inner').with_columns((pl.col('Survived') == pl.col('Survived_ground_truth')).alias('correct'))
